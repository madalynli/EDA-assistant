...
_calc_dataframe_statistics.py and _calc_variable_statistics.py. The tables produced are added into
...
def create_df_summary(df):
    ...
    df_summary = pd.DataFrame(summary_stats, columns=_format_tables.df_summary_labels, index=['Values'])
...
def create_var_summary(df):
    ...
    var_summary = pd.DataFrame(summary_stats, columns=_format_tables.var_summary_labels, index=var_summary_headers)
